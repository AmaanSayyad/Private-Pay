...
export default function StarknetPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // In production, you would derive an ephemeral key and proper stealth address using the recipient's meta address.
      // For now, we simulate usage by creating a dummy stealth address from the sender's ephemeral key.

      // Generate ephemeral key for this payment
      const ephemeralPrivKey = generatePrivateKey();

      // For demo: use RECIPIENT as spend key, but for now, recipient is a public key string, not meta-address object
      // If recipient is not a hex key, throw error
      let spendKey = recipient.trim();
      if (!/^0x[0-9a-fA-F]{64}$/.test(spendKey)) {
        toast.error("Recipient spend public key format invalid");
        setIsSending(false);
        return;
      }

      // For demo: split ephemeral public key (from private key bytes to hex)
      ...
      // In production: use generateStealthAddress(spendPublicKey, ephemeralPrivKey)
      ...
      const result = await sendPrivatePayment(
        // Use the stealth address as the recipient
        stealthAddressHash,
        stealthAddressHash,
        ephemeralX,
        ephemeralY,
      ...
      toast.success(
        `Payment sent! Tx: ${result.transaction_hash?.slice?.(0, 10) ?? ""}...`
      );
      setRecipient("");
      setAmount("");
    } catch (error) {
      ...
      <div className="w-full max-w-5xl">
        ...
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <p className="text-gray-600 max-w-xl text-sm text-center">
            Experience cross-chain privacy with Ztarknet - bridging Zcash shielded
            transactions with Starknet
          </p>
        </div>
                ...
                <div className="flex flex-col items-center gap-4 text-center">
                  ...
                  {!availableWallets.argentX && !availableWallets.braavos && (
                    <p className="text-xs text-center text-amber-600 mt-2">
                      No Starknet wallet detected. Install ArgentX or Braavos to
                      continue.
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="text-xl font-bold text-gray-800">
                      {balanceHidden ? "***" : balance.eth}{" "}
                      <span className="text-sm text-purple-500">ETH</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-purple-700 font-semibold flex items-center gap-1">
                    ...
                    <div className="text-xl font-bold text-gray-800">
                      {balanceHidden ? "***" : balance.strk}{" "}
                      <span className="text-sm text-purple-500">STRK</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="text-xl font-bold text-gray-800">
                      {balanceHidden ? "***" : balance.szec}{" "}
                      <span className="text-sm text-green-500">sZEC</span>
                    </div>
                    {!balanceHidden && balance.simulated > 0 && (
            ...
            <Card className="bg-white/80 backdrop-blur-xl border-2 border-white/50 shadow-2xl rounded-2xl overflow-hidden">
              <CardBody className="p-0">
                <Tabs
                  ...
                  classNames={{
                    tabList:
                      "gap-6 w-full relative rounded-none p-0 border-b-2 border-gray-200 px-6 pt-3 bg-gradient-to-r from-purple-50/50 to-indigo-50/50",
                    cursor: "bg-gradient-to-r from-purple-600 to-indigo-600 h-1",
                    tab: "max-w-fit px-0 h-12",
                    tabContent:
                      "group-data-[selected=true]:text-purple-600 group-data-[selected=true]:font-bold text-sm",
                  }}
                >
                  <Tab
                    key="receive"
                          ...
                          <Card className="bg-amber-50 border-2 border-amber-200">
                            <CardBody className="p-3">
                              <p className="text-xs text-amber-700">
                                <strong>Important:</strong> Your private keys
                                are stored locally. Back them up securely to avoid
                            ...
                            <Card className="bg-green-50 border-2 border-green-200">
                              <CardBody className="p-3">
                                <p className="text-xs text-green-700">
                                  ...
                                  >
                                    {metaAddress.txHash.slice(0, 10)}
                                    ...
                                    {metaAddress.txHash.slice(-6)}
                                  </a>
                                </p>
                              </CardBody>
                            </Card>
                    ...
                    <div className="p-6 flex flex-col gap-4 bg-gradient-to-br from-white to-indigo-50/30">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          classNames={{
                            inputWrapper:
                              "h-12 rounded-xl bg-white border-2 border-gray-200",
